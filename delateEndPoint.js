...
module.exports = function (MLModelId) {
  return new Promise(function (resolve, reject) {
    ...
    machinelearning.deleteRealtimeEndpoint({MLModelId: MLModelId}, function (error, resEndPoint) {
      if (error) {
        debug.ML.error('error on deleteRealtimeEndpoint', error)
        reject(error)
        return
      }
      ...
      if (resEndPoint.RealtimeEndpointInfo.EndpointStatus ===
        'NONE' || !resEndPoint) {
        debug.ML.info('resEndPoint on deleteRealtimeEndpoint', resEndPoint)
        resolve(resEndPoint)
        return
      }
      ...
      let timer = setInterval(function () {
        machinelearning.getMLModel({
          ...
          if (err) {
            debug.ML.info('error on deleteRealtimeEndpoint with timer', err)
            reject(err)
          } else if (res.EndpointInfo && res.EndpointInfo.EndpointStatus ===
            'NONE') {
              // the timer is removed if endpoint is removed
            clearInterval(timer)
            debug.ML.info('resEndPoint on deleteRealtimeEndpoint with timer', res)
            resolve(res)
          }
        })
      }, 60000)
