...
module.exports = function (resCreateModel, ScoreThreshold) {
  debug.ML.info('resCreateModel on getModelCompleted', resCreateModel)
  return new Promise(function (resolve, reject) {
    let timer = setInterval(function () {
      ...
      machinelearning.getMLModel(resCreateModel, function (err,
        ...
        debug.ML.info('resGetModel on getModelCompleted', resGetModel)
        if (err) {
          debug.ML.error('err on getModelCompleted', err)
          clearInterval(timer)
          reject(err)
          return
        }
        ...
        if (resGetModel.Status === 'DELETED' || resGetModel.Status ===
          ...
          if (ScoreThreshold && resGetModel.Status ===
            'COMPLETED' && resGetModel.MLModelType === 'BINARY') {
            ...
            delateEndPoint(
              ...
              .then(function () {
                ...
                machinelearning.updateMLModel({
                  ...
                  debug.ML.info('modelUpdated on getModelCompleted', modelUpdated)
                  // If error happen the promise is removed
                  if (err) {
                    debug.ML.error('err on getModelCompleted with updateMLModel', err)
                    reject(err)
                    return
                  }
          ...
          // The end point is created again
          getEndPoint(resGetModel.MLModelId)
            .then(resolve)
            .catch(reject)
        }
      }) /* required */
