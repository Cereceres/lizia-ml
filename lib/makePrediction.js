...
module.exports = function (datum, query) {
  debug.info('datum to make prediction is', datum)
  debug.info('query to find prediction is', query)
  query = query || {}
  let _query = {}
  ...
  return new Promise(function (resolve, reject) {
    ...
    MLmodel.find(_query).sort({updatedAt: -1}).exec(function (e, d) {
      if (e) debug.error('error in found a model', e)
      debug.info('model found to make prediction is',
      d[0] && d[0].toObject ? d[0].toObject() : d[0])
      if (!d.length || d[0].EndpointInfo.EndpointStatus !== 'READY') return resolve()
      ...
      machinelearning.predict({
        ...
        Record: datum
          ...
          function (err, prediction) {
            if (err) {
              debug.error('error on prediction is', err)
              reject(err)
            } else {
              debug.info('prediction is', prediction)
              resolve(prediction)
            }
          })
      return
