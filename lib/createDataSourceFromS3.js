...
function createDataSourceFromS3 (model, DataSourceId, DataSourceName, Bucket) {
  return new Promise(function (resolve, reject) {
    ...
    debug.info('model to create schema', model, Bucket)
    ...
    debug.info('params to be used in create datasourcefromS3', params)
    machinelearning.createDataSourceFromS3(params, function (err, datasource) {
      if (err) {
        debug.error('error en create datasouce is', err)
        ...
        if (err.message.search('already exists') > -1) {
          ...
          machinelearning.deleteDataSource({DataSourceId: DataSourceId
            ...
            if (error) {
              debug.error('error on remove the datasource', error)
              reject(error)
            } else {
              // The DataSourceId is updated
              DataSourceId = versioning(DataSourceId)
              debug.info('the new DataSourceId is updated to ', DataSourceId)
        ...
        debug.info('the data source created is', datasource)
