...
module.exports = function (MLModelId) {
  return new Promise(function (resolve, reject) {
    ...
    .createRealtimeEndpoint(
      ...
      error, resEndPoint) {
        debug.info('resEndPoint on getEndPoint', resEndPoint)
        if (error) {
          debug.error('error on getEndPoint', resEndPoint)
          reject(error)
          return
        }
        ...
        let timer = setInterval(function () {
          machinelearning.getMLModel({
            ...
            debug.info('res on getEndPoint', res)
            if (err) {
              debug.error('error on getEndPoint with timer', err)
              reject(err)
