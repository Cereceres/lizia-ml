...
module.exports = function (datum, query) {
  ...
  debug.ML.info('query to make prediction is', query)
  query = query || {}
  query.Status = 'COMPLETED'
  return new Promise(function (resolve, reject) {
    // The model is looking for
    MLmodel.find(query).sort({updatedAt: -1}).exec(function (e, d) {
      if (e) debug.ML.error('error in found a model', e)
      debug.ML.info('model found to make prediction is', d[0] && d[0].toObject ? d[0].toObject() : d[0])
